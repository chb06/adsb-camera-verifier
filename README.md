...
- **FFmpeg AVFoundation** preserved for the known-good macOS USB camera path
...
If OpenCV only sees the built-in Mac camera, install FFmpeg and use the known-good AVFoundation path:
...
### 2) Camera smoke test using the known-good Mac path

```bash
python -m app.smoke_camera \
...
```bash
python -m app.smoke_adsb --host adsbexchange.local --port 30003 --seconds 10
```
...
`configs/mac_dev.yaml` preserves the known-good Mac path:

- camera backend: `ffmpeg_avfoundation`
- camera source: `"Arducam B0498 (USB3 8.3MP)"`
...
```bash
python -m app.smoke_camera --backend ffmpeg_avfoundation --source "Arducam B0498 (USB3 8.3MP)" --mode 720p15 --display --seconds 20
python -m app.smoke_adsb --host adsbexchange.local --port 30003 --seconds 10
python -m app.run_realtime --config configs/default.yaml --override configs/mac_dev.yaml --seconds 60
```
